...
P0 = 1*eye(3);diag([(1*max(abs(x(1,:)'))).^2   (2*pi).^2  w.^2 ]);
...
Xckf_update = zeros(3,length(x_noisy(1,:)));
Xckf_predict = zeros(size(Xckf_update));
% CKF parameters
...
%The parameter alpha determines the spread of the sigma points around the centre, and
...
% weights of sigma points
w0m_state = 0;
wm_state = 1/(2*N_state);
w0c_state = 0;
wc_state = 1/(2*N_state);

w0m_measure = 0;
wm_measure = 1/(2*N_measure);
w0c_measure = 0;
wc_measure = 1/(2*N_measure);
...
X_sigma_state=zeros(length(X_initial),2*N_state); %space for sigma points for state space model
X_sigma_measure=zeros(3,2*N_measure); %space for sigma points for measurement model

X_propagate_sigma=zeros(length(X0),2*N_state); %space for propagated sigma points
...
P00 = diag([P_update_matrix(1,1,1) P_update_matrix(2,2,1) P_update_matrix(3,3,1) .01*R(1,1) (.1) (.1*RR_var)^2  ...
    (.05.*ones(1,size_params/3)).^2 (.0005.*ones(1,size_params/3)).^2 (.00005.*ones(1,size_params/3)).^2  ] );



X_propagate_sigma_matrix = zeros(length(X_initial),2*N_state,length(y(1,:)));
X_propagate_predict_sigma_matrix = zeros(length(X0),2*N_state,length(y(1,:)));
...
%% CKF filtering
for ii=1:size(Xckf_update,2)
    disp(['Forward CKF- Processing sample number:' num2str(ii) '/' num2str(size(Xckf_update,2))])
...
temp1 = y_propagate_sigma-repmat(y_propogate_sigma_mean,1,2*N_measure);
temp2 = X_sigma_measure-repmat(X0,1,2*N_measure);

P_xy1 = wc_measure*temp1*temp1';
P_xy2 = wc_measure*temp2*temp1';
...
if ii>(window_size+length(X0)+1)
    %           gamma(i) = mean(Memory_gamma(end-window_size+1:end-1));
...
P00 = diag([P_update_matrix(1,1,ii) P_update_matrix(2,2,ii) P_update_matrix(3,3,ii)  Q(1,1) (.1) (.1*RR_var)^2  ...
    (.005.*ones(1,size_params/3)).^2 (.00005*bi.*ones(1,size_params/3)).^2 (.0005.*ones(1,size_params/3)).^2]);



P00(1:3,1:3) = 1*P_update_matrix(1:3,1:3,ii);
...
% X00 = [X0;(0.0001*mean(ecgsd))^2;  (.00001); (.0001*RR_var)^2 ;ai';bi';tetai'];
X00 = [X0;(0.000001*rand*X0(1));  (.00001); (.0001*RR_var)^2 ;ai';bi';tetai'];
...
X_intermediate=mean(X_propagate_sigma,2);
...
temp3 = X_propagate_sigma(1:3,:)-repmat(X_intermediate(1:3),1,2*N_state);
P_xx = wc_state*temp3*(temp3');
...
Xcks = Xckf_update;
Pcks = P_update_matrix; 

for ii=size(Xckf_update,2)-1:-1:1
 
D = 0;
for nn=1:2*N_state    
D = D + wc_state*(X_propagate_predict_sigma_matrix(1:3,nn,ii+1)-Xckf_predict(1:3,ii+1))*(X_propagate_sigma_matrix(1:3,nn,ii)-Xckf_update(1:3,ii))';
end



L = D/(P_predict_matrix(:,:,ii+1));
Xcks(:,ii) = Xckf_update(:,ii) + L*(Xcks(:,ii+1)-Xckf_predict(:,ii+1));
if abs(Xcks(1,ii)-Xcks(1,ii+1))>20
    Xcks(:,ii)=Xckf_update(:,ii);
end
% Xcks(2,ii) = rem(Xcks(2,ii)+pi,2*pi)-pi;
Pcks(:,:,ii) = P_update_matrix(:,:,ii) + L*(Pcks(:,:,ii+1)-P_predict_matrix(:,:,ii+1))*L';
disp(['Backward CKS- Processing sample number:' num2str(ii) '/' num2str(size(Xckf_update,2))])
...
ACKF_SNR = 10*log10(mean((x(1,:)-y(1,:)).^2)/mean((x(1,:)-Xckf_update(1,:)).^2))
ACKS_SNR = 10*log10(mean((x(1,:)-y(1,:)).^2)/mean((x(1,:)-Xcks(1,:)).^2))
...
subplot(3,1,1)
        ...
        title([file])
        subplot(3,1,3)
        plot(1:length(x),Xckf_update(1,:),'r')
                legend({'Adaptive CKF'})
                        axis('tight')
